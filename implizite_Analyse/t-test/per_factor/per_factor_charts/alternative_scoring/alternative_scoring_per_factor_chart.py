...
def main():
    # Define file paths.
    input_csv = "implizite_Analyse/t-test/per_factor/alternative_scoring_all_ttests_results.csv"
    output_dir = "implizite_Analyse/t-test/per_factor/per_factor_charts/alternative_scoring"
    ...
    # --- New block: Print every test for languages ---
    # Filter tests where "Grouping Factor" contains the word "language" (case-insensitive).
    language_tests = df[df["Grouping Factor"].str.lower().str.contains("language", na=False)]
    print("\nTests for languages:")
    # Print all language tests (each row). The expected count is 9.
    print(language_tests.to_string(index=False))
    print(f"Total language tests: {len(language_tests)}")
